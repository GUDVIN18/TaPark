...
async def intent_classifier(state: SupportAi) -> SupportAi:
    ...
    * использует формулировки «как изменить», «как удалить», «как добавить», «как настроить», «как исправить», «где найти».
    ...
    Важно: вопрос о том, как пользователь может выполнить действие самостоятельно, всегда относится к KNOWLEDGE, если тема присутствует в схеме базы знаний.
    ...
    * сообщает о конфликте, жалобе или спорной ситуации, требующей решения сотрудника;
    * сообщает, что инструкция не помогла и ему требуется помощь специалиста.

    Не выбирай OPERATOR только из-за наличия слов «изменить», «удалить», «увольнение», «исправить» или «настроить». Сначала определи, пользователь спрашивает инструкцию или просит выполнить действие за него.
    ...
    * задаёт уточнение, ответ на которое полностью содержится в истории.
    ...
    3. Вопрос о том, как выполнить действие самостоятельно — KNOWLEDGE.
    4. Вопрос относится к разделу или подразделу из схемы базы знаний — KNOWLEDGE.
    5. Для ответа достаточно истории или обычного общения — CHAT.
    6. Если есть сомнение между KNOWLEDGE и OPERATOR — выбирай KNOWLEDGE.
    7. Если есть сомнение между KNOWLEDGE и CHAT — выбирай KNOWLEDGE.
...
async def dynemic_rag_context(state: SupportAi) -> SupportAi:
    ...
    2. Обычно выбирай 1-3 основные разделы и 1-4 самый релевантный подраздел.
