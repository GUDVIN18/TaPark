...
async def intent_classifier(state: SupportAi) -> SupportAi:
    ...
    * спрашивает, где находится функция, кнопка, раздел или с чего начать;
...
async def dynemic_rag_context(state: SupportAi) -> SupportAi:
    ...
    У тебя есть дерево базы знаний: разделы `chapters`, их подразделы — в
    `children`, а следующие уровни вложенности — в `children` соответствующего узла.
    ...
    Правила выбора:
        ...
        4. Если вопрос точно совпадает с названием подраздела любого уровня, верни его
        название в children внутри верхнего раздела. Для поиска допустимо выбрать
        название дочерней, вложенной или самой глубокой темы.
...
async def classify_operator_confirmation(state: SupportAi) -> SupportAi:
    ...
    prompt_template = PromptTemplate(
        template="""
    Ты классифицируешь текущее сообщение пользователя. Твоя задача — понять, подтверждает ли он создание заявки, отказывается или задает новый вопрос.

    Доступные категории (form_type):
    - CONFIRMED: Пользователь подтверждает создание заявки (например: "да", "ага", "хочу", "подтверждаю", "создавай", "окей", "давай").
    - DECLINED: Пользователь отказывается от создания заявки (например: "нет", "не надо", "отмена").
    - NEW_QUESTION: Пользователь задал новый вопрос или написал сообщение, не относящееся к подтверждению заявки.
